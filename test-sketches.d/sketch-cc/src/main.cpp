...
#define lcl_printf() print_buffering();
...
void refresh_LED_array() { do_a_thing(); }
...
byte reading() {
    ...
    ch = Serial.read();
    Serial.write(
        ch); // for current environment - SerialMonitor may double-print on this
    if (ch == '\r')
        return 1;
    if (ch == '\n')
        return 0;
...
void print_buffering(void) { Serial.print(buffering); }
...
void test_program_a(void) {
    ...
    */
    ...
    // related to string length, possibly snprintf(buffer, sizeof(buffer),
    // "%d%c", 27, '\000'); // related to string length, possibly lcl_printf();
    // --------------------------  crashed --------------------------------
    // --------------------------  crashed --------------------------------
    // --------------------------  crashed --------------------------------
    // --------------------------  crashed --------------------------------
    ...
    */
    ...
    // buf_ptr = buffer;
    // snprintf(buf_ptr, sizeof(buf_ptr), "%s", "         strlen(buffer)  is ",
    // '\000'); lcl_printf();
    /* buf_ptr = * & buffer;
    memcpy(buffering, buf_ptr, sizeof buffer);
    lcl_printf();
